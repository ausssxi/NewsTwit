...
$yesterday = strtotime('-1 day' . $_GET["date"]);
$nextday = strtotime('+1 day' . $_GET["date"]);
$today = date("Y").date("m").date("d");
$day_29 = date("Y", strtotime('-29 day')).date("m", strtotime('-29 day')).date("d", strtotime('-29 day'));
...
head("${year}年${month}月${day}日のコメントランキング（${category}）", "entame");
header_html();
toggle();
$result = $mysqli->query("SELECT COUNT(*) FROM ranking_today WHERE category = $_GET[category] AND date = $_GET[date]");
...
if ($ranking_count == 0) {
    ...
    echo "<ul class='flex list-style-type-none width-718-1078 margin-48-auto-0 padding-left-0 font-size-12 nowrap'>
        ...
        <li class='breadcrumb line-height-1'><a href='https://newstwit.jp/ranking/today/$_GET[category]/${year}${month}${day}' class='color-black underline'><strong>${year}年${month}月${day}日のコメントランキング（${category}）</strong></a></li>
    </ul>
            ...
            <div class='radius-8 background-white padding-16'>
                <div class='flex space-between align-items-center'>
                    <div class='font-size-14'>";
                        if ($today > $_GET["date"]) {
                            echo "<a class='color-black underline' href='/ranking/today/$_GET[category]/" . date("Y", $nextday) . date("m", $nextday) . date("d", $nextday) . "'>＜＜&nbsp;翌日</a>";
                        }
                    echo "</div>
                    <div class='text-align-center'>
                        ...
                        <div class='font-size-14'>
                            <strong>${year}年${month}月${day}日付</strong>
                        </div>
                    </div>
                    <div class='font-size-14'>";
                        if ($_GET["date"] > $day_29) {
                            echo "<a class='color-black underline' href='/ranking/today/$_GET[category]/" . date("Y", $yesterday) . date("m", $yesterday) . date("d", $yesterday) . "'>前日&nbsp;＞＞</a>";
                        }
                    echo "</div>
                </div>
            </div>";
            $i = 1;
            $result_count = $mysqli->query("SELECT news_id, count FROM ranking_today WHERE category = $_GET[category] AND date = $_GET[date]");
