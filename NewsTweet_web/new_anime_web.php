...
$result = $mysqli->query("SELECT COUNT(*) FROM `news` WHERE news_id in (SELECT news_id FROM news_to_tag WHERE tag_id = 9082120)");
$row = $result->fetch_row();
$news_count = $row[0];
head("アニメ・ゲーム（新着）", "new/anime");
header_html();
toggle();
new_main($news_count, "アニメ・ゲーム", "anime", 3, "SELECT news_id, title, url, image, description, site_domain_name, up FROM news WHERE news_id in (SELECT news_id FROM news_to_tag WHERE tag_id = 9082120) ORDER BY news_id DESC LIMIT 40");
ads_728();
footer();
ads_320();
?>
        ...
        $(function () {
            ...
            function more() {
                if (count > 0) {
                    ...
                    $.get("/new_anime_more_web.php", getData, function(rs) {
                        $(".news-read").append(rs);
