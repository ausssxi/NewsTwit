            ...
            if len(row[2]) < 500:
                ...
                for tweet in tweets:
                    ...
                    if text != "" and len(text) > 1 and comp_a not in comp_b:
                        ...
                        print(row[4])
                        print(tweet.id)
                        Datatime = tweet.created_at + datetime.timedelta(hours=9)
                        try:
                            cursor.execute("INSERT INTO comment (comment_id, news_id, twitter_id, tweet, name, screen, icon, up) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (tweet.id, row[0], tweet.user.id, text, tweet.user.name, tweet.user.screen_name, tweet.user.profile_image_url_https, Datatime))
                            conn.commit()
                        except:
                            pass
    except:
